...
from maze3D_new.layouts import layout_10_down_right, layout_10_up_left, layout_10_up_right
# RL modules
from plot_utils.plot_utils import get_config

# the game layouts
# layouts = [layout_down_right, layout_up_left, layout_up_right]
# layouts = [layout_10_up_right]
layouts = [layout_10_down_right, layout_10_up_left, layout_10_up_right]
...
class Maze3D:
    ...
    def step(self, action_agent, timed_out, goal, action_duration):
        ...
        while (time.time() - start_time - current_duration_pause) < action_duration and not self.done:
            ...
            current_duration_pause, _, self.human_actions = self.getKeyboard(actions)
...
class Maze3DGym(Maze3D):
    def __init__(self, config=None, config_file=None):
        super().__init__(config, config_file)
        self.action_space.actions = [0, 1, 2, 3, 4]
        self.action_space.actions_number = len(self.action_space.actions)
    
    def step(self, agent_action, timed_out, goal, action_duration):
        """
        Performs the action of the agent to the environment for action_duration time.
        Simultaneously, receives input from the user via the keyboard arrows.
        :param action_agent: the action of the agent. make sure it is compatible. if None human used both axes
        ...
        while (time.time() - start_time - current_duration_pause) < action_duration and not self.done:
            ...
            current_duration_pause, _, self.human_actions = self.getKeyboard(actions)
            duration_pause += current_duration_pause           
            action = self.transform_agent_action(agent_action)
            action_list.append(action)
    ...
    def transform_agent_action(self, agent_action):
            # no action: 0, up: 1, down:2, left:3, right:4, upleft:5, upright:6, downleft: 7, downright:8
            if agent_action == 0:
                return [0, 0]
            elif agent_action == 1:
                return [1, 0]
...
class Maze3DCollaborative(Maze3D):
    def __init__(self, config=None, config_file=None):
        super().__init__(config, config_file)
        self.action_space.actions = [0, 1, 2]
        self.action_space.actions_number = len(self.action_space.actions)

    def step(self, agents_actions, timed_out, goal, action_duration):
        """
        Performs the action of the agent to the environment for action_duration time.
        Simultaneously, receives input from the user via the keyboard arrows.
        :param action_agent: the action of the agent. make sure it is compatible. if None human used both axes
        ...
        while (time.time() - start_time - current_duration_pause) < action_duration and not self.done:
            ...
            agents_actions = [action-3 if action == 2 else action for action in agents_actions]

            action_list.append(agents_actions)

            self.board.handleKeys(agents_actions)  # apply action to the environment
            self.board.update()  # update board's rotations
            glClearDepth(1000.0)
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
            self.board.draw()  # render new graphics of the game
