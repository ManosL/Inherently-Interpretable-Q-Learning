...
class GameBoard:
    ...
    def draw(self, mode=0, idx=0):
        ...
        # Used for resetting the game. Logs above the board "Timeout"
            ...
            glDrawArrays(GL_TRIANGLES, 0, TEXT_MODEL.getVertexCount())
        # Used for the user's action estimation
        # elif mode == 4:
        #     translation = pyrr.matrix44.create_from_translation(pyrr.Vector3([-90, -450, 0]))
        #     glUniformMatrix4fv(MODEL_LOC, 1, GL_FALSE,
        #                        pyrr.matrix44.multiply(translation, pyrr.matrix44.create_identity()))
        #     glBindVertexArray(HOLE_MODEL.getVAO())
        #     glBindTexture(GL_TEXTURE_2D, EXPLAIN[0].getTexture())
        #     glDrawArrays(GL_TRIANGLES, 0, HOLE_MODEL.getVertexCount())
        elif mode == 5:
            translation = pyrr.matrix44.create_from_translation(pyrr.Vector3([-400, -150, 0]))
            glUniformMatrix4fv(MODEL_LOC, 1, GL_FALSE,
                               pyrr.matrix44.multiply(translation, pyrr.matrix44.create_identity()))
            glBindVertexArray(CHERRY_MODEL.getVAO())
            glBindTexture(GL_TEXTURE_2D, CHERRY.getTexture())
            glDrawArrays(GL_TRIANGLES, 0, CHERRY_MODEL.getVertexCount())
        elif mode == 6:
            translation = pyrr.matrix44.create_from_translation(pyrr.Vector3([400, -150, 0]))
            glUniformMatrix4fv(MODEL_LOC, 1, GL_FALSE,
                               pyrr.matrix44.multiply(translation, pyrr.matrix44.create_identity()))
            glBindVertexArray(APPLE_MODEL.getVAO())
            glBindTexture(GL_TEXTURE_2D, APPLE.getTexture())
            glDrawArrays(GL_TRIANGLES, 0, APPLE_MODEL.getVertexCount())
