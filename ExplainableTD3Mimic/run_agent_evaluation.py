...
def evaluate_agent_in_episode(agent, env, max_steps, on_explainable_model):
    ...
    while not done and steps < max_steps:
        action = agent.choose_action(observation, from_target_actor=on_explainable_model)
...
def main(args):
    ...
    agent = ExplainableTD3MimicAgent(actor_lr=0.001, critic_lr=0.001, tau=0.05, input_dims=env.observation_space.shape, 
                                     env=env, n_actions=env.action_space.shape[0], chkpt_dir=args.model_dir, target_model_type='gbr',
                                     gbr_estimators=500, gbr_max_depth=5,
                                     gbr_learning_rate=0.5, gbr_subsample=1.0,
                                     gbr_min_leaf_samples=100)

    agent.load_models()

    for j in range(1, args.episodes + 1):
        score = evaluate_agent_in_episode(agent, env, args.max_episode_duration, args.on_explainable_model)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--on_explainable_model', action='store_true', default=False,
                        help='If present, then we evaluate the agent by choosing its action from the \
                            explainable target policy model instead of the primary actor network.')
